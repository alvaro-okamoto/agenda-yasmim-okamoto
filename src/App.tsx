...
const companyBlocksSeed: CompanyBlock[] = [
  {
    ...
    active: true,
    shifts: [
      { id: "s1", weekday: 3, startTime: "13:00", endTime: "18:00" },
      { id: "s2", weekday: 5, startTime: "08:00", endTime: "12:00" },
    ],
  },
  {
    id: "c2",
    ...
    active: true,
    shifts: [{ id: "s3", weekday: 1, startTime: "08:00", endTime: "11:30" }],
  },
];
...
export default function App() {
        ...
        .forEach((block) => {
          block.shifts
            ...
            .forEach((shift) => {
              ...
              result.push({
                id: `company-${block.id}-${shift.id}-${dateStr}`,
                kind: "company",
                date: dateStr,
  ...
  function startEditCompany(block: CompanyBlock) {
    ...
    setCompanyForm({
      ...
      notes: block.notes,
      active: block.active,
      shifts: block.shifts.map((shift) => ({ ...shift })),
    });
  }
  ...
  function updateCompanyShift(shiftId: string, patch: Partial<CompanyShift>) {
    setCompanyForm((prev) => ({
      ...prev,
  ...
  function addCompanyShift() {
    setCompanyForm((prev) => ({
      ...
      shifts: [
        ...prev.shifts,
        { id: nextId("shift"), weekday: 1, startTime: "08:00", endTime: "12:00" },
      ],
    }));
  }
  ...
  function saveCompanyForm() {
    ...
    if (companyForm.shifts.length === 0) {
      showToast("error", "Adicione pelo menos um dia de atendimento.");
      return;
    }

    const invalidShift = companyForm.shifts.some(
      (shift) => timeToMinutes(shift.endTime) <= timeToMinutes(shift.startTime)
    );

    if (invalidShift) {
      showToast("error", "A hora final deve ser maior que a hora inicial em todos os turnos.");
      return;
    }
        ...
        {selectedCompanyBlock ? (
          <div style={{ display: "grid", gap: 12 }}>
            ...
            <div style={styles.infoBox}>
              <div style={{ fontWeight: 700, marginBottom: 8 }}>Dias e horários desta rotina</div>
              <div style={{ display: "grid", gap: 8 }}>
                {selectedCompanyBlock.shifts.map((shift) => (
                  <div key={shift.id} style={styles.miniInfoButton}>
                    <div style={{ fontWeight: 700 }}>
                      {weekdayFullLabels[shift.weekday]}
                    </div>
                    <div style={{ color: colors.subtext, fontSize: 13 }}>
                ...
                <div style={{ display: "grid", gap: 12 }}>
                  ...
                  <div style={styles.infoBox}>
                    <div style={{ ...styles.rowBetween, marginBottom: 12 }}>
                      <div style={{ fontWeight: 700 }}>Dias e horários da rotina</div>
                      <button style={styles.secondaryButton} onClick={addCompanyShift}>
                        Adicionar dia
                      </button>
                    </div>
                      ...
                      {companyForm.shifts.map((shift, index) => (
                        <div key={shift.id} style={styles.shiftCard}>
                          <div style={{ ...styles.rowBetween, marginBottom: 10 }}>
                            <div style={{ fontWeight: 700 }}>Turno {index + 1}</div>
                            {companyForm.shifts.length > 1 ? (
                              <button
                                style={styles.deleteButton}
                                onClick={() => removeCompanyShift(shift.id)}
                              >
                                Remover
                              </button>
                            ) : null}
                  ...
                  <div key={block.id} style={styles.companyManagerCard}>
                    <div style={styles.rowBetween}>
                      <div style={{ textAlign: "left" }}>
                        ...
                        <div style={{ color: colors.subtext, fontSize: 12, marginTop: 8, display: "grid", gap: 4 }}>
                        ...
                        <div style={{ color: colors.subtext, fontSize: 12, marginTop: 8 }}>
